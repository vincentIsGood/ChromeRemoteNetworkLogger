...
import DomNavigator from "./domnav/DomNavigator.mjs";
...
class NetworkLogger{
    ...
    constructor(url, context, isIframe = false, requestStore = new RequestStore()){
        LOGGERS.set(context.config.target, this);
        this.url = url;
    ...
    async setup(includeIframes = false){
        ...
        //     //     this.internalBuffer.push(e);
        // });

        // Setup handlers (note that ExtraInfo may come in first)
        //
        // buffer all objects instead of processing them. We will lose packets by
        // processing it first
        ...
        Network.on("requestWillBeSent", (e)=>{
            ...
            // TODO: Old workaround to setAutoAttach
            // if(NetworkLogger.isNotSimpleRequestId(requestId)){
            //     for(let i = 1; i <= 5; i++){
            //         setTimeout(()=>{
        ...
        Network.on("dataReceived", (e)=>{
            ...
            Network.getResponseBody({requestId: e.requestId}).then(res => {
        ...
        Target.on("attachedToTarget", (e)=>{
            console.log("Attached to target: ", e);
            if(includeIframes){
                this.captureIframeNetworkTraffic();
            }
        })
     ...
     * @param {string} reqId 
     */
    static isNotSimpleRequestId(reqId){
        return reqId.length >= 32;
    }
}
